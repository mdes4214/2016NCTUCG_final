...
GLuint createProgram(GLuint vert, GLuint frag)
{
	...
	glAttachShader(currentProgram, vert);
	glAttachShader(currentProgram, frag);
	...
	GLint isLinked = 0;
	glGetProgramiv(currentProgram, GL_LINK_STATUS, &isLinked);

	if (isLinked == GL_FALSE)
	{
		GLint maxLength = 0;
		glGetProgramiv(currentProgram, GL_INFO_LOG_LENGTH, &maxLength);

		//The maxLength includes the NULL character
		char *infoLog = (char *)malloc(sizeof(char) * (maxLength));
		glGetProgramInfoLog(currentProgram, maxLength, &maxLength, infoLog);
	...
	glDetachShader(currentProgram, vert);
	glDetachShader(currentProgram, frag);
